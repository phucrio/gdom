...
impl SqliteAccountStore {
    ...
    async fn from_options(options: SqliteConnectOptions) -> Result<Self, AccountStoreError> {
        ...
        let row: Option<(i64,)> =
            sqlx::query_as("SELECT version FROM _schema_migrations WHERE version = 5")
                .fetch_optional(&pool)
                .await?;
        if row.is_none() {
            let mut tx = pool.begin().await?;
            sqlx::raw_sql(include_str!("../../migrations/005_canary_selected.sql"))
                .execute(&mut *tx)
                .await?;
            sqlx::query(
        ...
        let row: Option<(i64,)> =
            sqlx::query_as("SELECT version FROM _schema_migrations WHERE version = 6")
                .fetch_optional(&pool)
                .await?;
        if row.is_none() {
            let mut tx = pool.begin().await?;
            sqlx::raw_sql(include_str!("../../migrations/006_worker_leases.sql"))
                .execute(&mut *tx)
                .await?;
            sqlx::query(
