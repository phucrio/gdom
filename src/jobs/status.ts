import type { JobRunPhase, JobStatus } from "../ipc/types.ts";

export const DEFAULT_JOB_RUN_PHASE: JobRunPhase = "scan";

export function jobRunPhase(job: { phase?: JobRunPhase } | null): JobRunPhase {
  return job?.phase ?? DEFAULT_JOB_RUN_PHASE;
}
...
export function canaryAllowsBulk(status: JobStatus, phase: JobRunPhase = DEFAULT_JOB_RUN_PHASE): boolean {
  if (status === "QUEUED") {
    return phase === "bulk";
  }
  if (status === "PAUSED") {
    return phase === "canary" || phase === "bulk";
  }
  switch (status) {
    case "CANARY_REVIEW":
    case "RUNNING":
...
export function canResumeScan(
  ...
  phase: JobRunPhase = DEFAULT_JOB_RUN_PHASE,
): boolean {
  return status === "PAUSED" && phase === "scan";
}

export function canStartCanary(
  status: JobStatus | null,
  phase: JobRunPhase = DEFAULT_JOB_RUN_PHASE,
): boolean {
  return status === "READY_FOR_REVIEW" || (status === "QUEUED" && phase === "canary");
}
...
export function canStartBulk(
  status: JobStatus | null,
  phase: JobRunPhase = DEFAULT_JOB_RUN_PHASE,
): boolean {
  return status === "CANARY_REVIEW" || (status === "QUEUED" && phase === "bulk");
}
